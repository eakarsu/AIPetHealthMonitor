...
// AI customizes recommendations based on genetic disease risk, age-related decline (arthritis, hearing loss); proactive prevention suggestions
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPetHealthMonitor / cf-breedage-specific-care-automation',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_breedage_specific_care_automation`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-breedage-specific-care-automation feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Breed/age-specific care automation'.
Context: AI customizes recommendations based on genetic disease risk, age-related decline (arthritis, hearing loss); proactive prevention suggestions
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-breedage-specific-care-automation/run error:', err);
