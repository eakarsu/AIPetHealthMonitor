...
// Gap Feature: No veterinary clinic integration (medical records import, appointment booking)
// No veterinary clinic integration (medical records import, appointment booking)
// Project: AIPetHealthMonitor
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPetHealthMonitor / gap-no-veterinary-clinic-integration-medical-records-i',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No veterinary clinic integration (medical records import, appointment booking)'.\nProject: AIPetHealthMonitor.\nContext: No veterinary clinic integration (medical records import, appointment booking)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-veterinary-clinic-integration-medical-records-i', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-veterinary-clinic-integration-medical-records-i', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['no-veterinary-clinic-integration-medical-records-i']
    );
