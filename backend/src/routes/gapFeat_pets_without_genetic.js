...
// Gap Feature: Pets without \'/genetic
// Pets without /genetic-disease-risk (breed predisposition analysis)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPetHealthMonitor / gap-pets-without-genetic',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Pets without \'/genetic'.\nProject: AIPetHealthMonitor.\nContext: Pets without /genetic-disease-risk (breed predisposition analysis)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['pets-without-genetic', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'pets-without-genetic', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
