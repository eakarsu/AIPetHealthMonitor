...
// Real-time assessment of emergency severity; AI routes to 24-hr clinic vs. wait for morning vet appointment vs. poison control
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPetHealthMonitor / cf-emergency-decision-support',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_emergency_decision_support`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-emergency-decision-support feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Emergency decision support'.
Context: Real-time assessment of emergency severity; AI routes to 24-hr clinic vs. wait for morning vet appointment vs. poison control
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-emergency-decision-support/run error:', err);
