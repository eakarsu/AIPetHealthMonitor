...
// Regular full-body pet photos; AI detects changes (weight, fur quality, skin condition, discharge); auto-alerts on concerning changes
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPetHealthMonitor / cf-photo-based-health-screening',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_photo_based_health_screening`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-photo-based-health-screening feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Photo-based health screening'.
Context: Regular full-body pet photos; AI detects changes (weight, fur quality, skin condition, discharge); auto-alerts on concerning changes
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-photo-based-health-screening/run error:', err);
