...
// AI tracks regional vet pricing; suggests clinics, negotiates referrals based on cost + quality
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPetHealthMonitor / cf-veterinary-cost-negotiation',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_veterinary_cost_negotiation`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-veterinary-cost-negotiation feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Veterinary cost negotiation'.
Context: AI tracks regional vet pricing; suggests clinics, negotiates referrals based on cost + quality
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-veterinary-cost-negotiation/run error:', err);
