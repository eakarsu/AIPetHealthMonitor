...
// Gap (Non-AI): Backend collapses everything into crud.js
// Backend collapses everything into crud.js — no modular per-domain routes
import React, { useState } from 'react';

export default function GapBackendCollapsesEverythingIntoCrudJsPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-backend-collapses-everything-into-crud-js/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Backend collapses everything into crud.js</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Backend collapses everything into crud.js — no modular per-domain routes</p>
