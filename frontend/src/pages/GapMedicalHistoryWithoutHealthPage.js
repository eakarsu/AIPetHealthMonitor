...
// Gap (AI): Medical history without '/health
// Medical history without /health-trend-detect (flag chronic issues developing)
import React, { useState } from 'react';

export default function GapMedicalHistoryWithoutHealthPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-medical-history-without-health/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Medical history without '/health</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Medical history without /health-trend-detect (flag chronic issues developing)</p>
