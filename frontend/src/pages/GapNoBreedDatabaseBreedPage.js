...
// Gap (Non-AI): No breed database (breed
// No breed database (breed-specific health risks)
import React, { useState } from 'react';

export default function GapNoBreedDatabaseBreedPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-breed-database-breed/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No breed database (breed</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No breed database (breed-specific health risks)</p>
