...
// Gap (Non-AI): No webhooks for clinic events
// No webhooks for clinic events
import React, { useState } from 'react';

export default function GapNoWebhooksForClinicEventsPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-webhooks-for-clinic-events/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No webhooks for clinic events</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No webhooks for clinic events</p>
