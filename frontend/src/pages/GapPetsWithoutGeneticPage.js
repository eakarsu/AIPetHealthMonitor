...
// Gap (AI): Pets without '/genetic
// Pets without /genetic-disease-risk (breed predisposition analysis)
import React, { useState } from 'react';

export default function GapPetsWithoutGeneticPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-pets-without-genetic/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Pets without '/genetic</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Pets without /genetic-disease-risk (breed predisposition analysis)</p>
